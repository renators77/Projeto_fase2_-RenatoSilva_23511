...
processPlan *inicializarProcessPlan() {
	...
	if(processPlanEmMemoria==NULL) {
		...
		(*processPlanEmMemoria).allJobs = NULL;
...
void verificarDadosNoFicheiroTodos(processPlan *pPlan, operacao *op, int *idCont, int *nOperacoes) {
	int jobInt = 0;
	jobInt++;
		...
		if(fileProcessPlan != NULL) {
			do {
				if((symb = getc(fileProcessPlan)) != EOF) {
					...
					allPlan=(processPlan *)malloc(sizeof(processPlan));
					(*allPlan).allJobs=NULL;
					...
					(*op).id = (*idCont);
					(*op).qttMaquinas = cont;
					(*op).MaquinaTempo = (int *)malloc(sizeof(int[2][cont]));//[x,y]
...
void verificarDadosNoFicheiro(operacao *op, int *idCont, int *nOperacoes) {
	operacao *opP; 
	operacao *opEmMem;
	FILE *fileProcessPlan = fopen("ProcessPlan.txt","r");
	char symb ;
    unsigned char symbI;
	int i = 0; 
	int cont = 0; 
...
void guardarDados(operacao *op, int nOperacoes) {
	...
	if((*op).next==NULL)	{
		...
		FILE *f = fopen("ProcessPlan.txt", "w");
		if (f == NULL)
		{
		...
		while((*op).next != NULL) {
			...
			for (j = 0; j < (*op).qttMaquinas; ++j) {
				if(((*op).qttMaquinas - j) == 1)
					fprintf(f,"%d",(*op).MaquinaTempo[0*(*op).qttMaquinas + j]);
				else
					fprintf(f,"%d,",(*op).MaquinaTempo[0*(*op).qttMaquinas + j]);
			}
			fprintf(f,")\n[");
			for (j = 0; j < (*op).qttMaquinas; ++j) {
				if(((*op).qttMaquinas - j) == 1)
					fprintf(f,"%d",(*op).MaquinaTempo[1*(*op).qttMaquinas + j]);
				else
					fprintf(f,"%d,",(*op).MaquinaTempo[1*(*op).qttMaquinas + j]);
			}
			fprintf(f,"]\n");
...
void InserirJob (processPlan *pPlan, int *idJob){
	...
	int j;
	int f;
	processPlan *processPlanEmMem;
	processPlanEmMem = (processPlan *)malloc(sizeof(processPlan));
	
	system("cls");
	if(processPlanEmMem==NULL) { 
		printf("Nao existe mais espaço na memoria. Elimine alguns Jobs! \n\n");
		system("pause");
	}
	else {
		system("cls");
        (*processPlanEmMem).next=NULL;
...
int jobInt;
	...
	printf("Qual job pretende eliminar: ");
	fgets(jobInt, sizeof(jobInt), stdin); // 4

	if(jobInt > nJobs){
		printf("O valor inserido não existe");
		system("pause");
	}else{
